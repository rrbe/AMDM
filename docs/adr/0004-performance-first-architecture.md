...
5. **Only one bounded auto-query on first collection open.** Run `find({}).sort({ _id: -1 }).limit(100)` once; later clicks focus the existing Shell tab without re-running it.
