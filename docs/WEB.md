...
开发时直接运行，React 使用 Vite HMR；Vite 监听所有网络接口，并由开发中间件注入固定的 `local-dev` 身份：

```bash
pnpm dev:web
```

本机访问 `http://127.0.0.1:5173`，也可以通过 `http://<Tailscale IP>:5173` 访问。本地凭据密钥和状态自动保存在已忽略的 `.web-data/`。所有开发访问共享同一个 `local-dev` 身份，因此只应暴露在可信网络中。
