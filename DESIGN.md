...
- **Data grids own selection — no native text selection.** Structured result surfaces (the **Tree** and **Table** views) carry their own selection model, not the browser's. A plain click selects: in the **Tree**, a nested node, or a whole top-level document (`Shift` = range, `⌘`/`Ctrl` = toggle across documents); in the **Table**, the whole row, with the clicked cell getting an *overlay* highlight on top of the row tint (`Shift` = row range, `⌘`/`Ctrl` = toggle; **double-click edits** the cell). No modifier is required — a single click already selects. `⌘C` copies the selection (one doc/row as an object, several as a JSON array); right-click offers structured copy (Pure JSON / Shell JS / Extended JSON). Because the app owns selection, native browser text selection is **suppressed** there (`user-select: none` on `.kv-row` / `.table-scroller`). Native rubber-band selection on a grid is a web-page tell: it paints `::selection` tint as disconnected rectangles over each short token (the `(n)` badge, the `{ n fields }` summary, the type label), reading as a half-selected page rather than a clean row — and it copies the *rendered, ellipsis-truncated* text, not the real data. The inline cell editor opts back in (`user-select: text` on `.cell-edit-input`). Surfaces that are genuinely free text — the **JSON** result view, the query editor, error toasts, inputs — keep native selection.
