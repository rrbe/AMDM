...
### UI 组件（Base UI 封装层 + `data-*` 上色约定）
chrome 层（弹窗 / 下拉 / 菜单 / 复选 / Tabs / 表单）建立在 **headless 库 [`@base-ui/react`](https://base-ui.com)** 之上，由它负责行为、无障碍、焦点管理与表单校验；视觉仍用现有 Slate token 上色，**零运行时样式**。

- **薄封装是硬约定。** 只有 `components/ui/*` 直接 `import '@base-ui/react/...'`；全 app 其余地方一律 import 我们自己的 `ui/*` 封装，**绝不**直接碰 Base UI——隔离它的 API 变动（库尚年轻，`package.json` 锁精确版本 `1.5.0`）。封装层文件：

  | 封装 | 基于 | 替换/支撑 | 复用的 class |
  |---|---|---|---|
  | `ui/Dialog` | Dialog | `common/Modal` 内核、`.url-popup` 嵌套弹窗 | `.modal*` / `.url-popup*` |
  | `ui/Select` | Select | 原生 `<select>`（8 处） | `.ui-select-*`（trigger 仿 input，popup 仿 `.ctx-menu`） |
  | `ui/Field` + `ui/Input` | Field / Input | `.form-row` 标签输入行 | `.form-row` / `label` / `.hint` / `.field-err` |
  | `ui/NumberField` | NumberField | 数值项（每页条数 / 字号） | `.ui-number-*` |
  | `ui/Checkbox` | Checkbox | 原生 `<input type=checkbox>` | `.ui-check-*` |
  | `ui/Menu` | Menu | `ContextMenu`（光标虚拟锚点定位） | `.ctx-*` |
  | `ui/Tabs` | Tabs | `ConnectionForm` 的 `.tabs` 按钮组 | `.tabs` / `.tabs button` |
  | `ui/Tooltip` | Tooltip | （备用，暂未接入；现仍用 `TooltipLayer` 单例） | `.app-tooltip` |

- **上色走 `className` + `data-*`。** Base UI 在元素上落状态属性，CSS 用属性选择器上色，而非靠组件 prop：
  - 通用：`[data-disabled]`、`[data-open]`/`[data-closed]`、`[data-side]`、`[data-starting-style]`/`[data-ending-style]`（动画）。
  - Select 项：`[data-highlighted]`（键盘/hover 高亮）、`[data-selected]`。
  - Checkbox：根上 `[data-checked]`（驱动方框填充）。
  - Tabs：`[data-selected]`（取代旧的 `.active` class）。
  - Field：`[data-invalid]`/`[data-valid]`/`[data-touched]`/`[data-dirty]`/`[data-filled]`。
- **弹层叠放：** Base UI 把弹层 `Portal` 到 `<body>`；`#root` 设 `isolation: isolate` 建独立层叠上下文。z-index 层级：弹窗遮罩 `1000` < 弹窗 `1001` < `.url-popup` 嵌套弹窗 `1101` < select/菜单定位器 `2000` < tooltip `3000`。复用 `.ctx-menu`/`.app-tooltip` 的弹层把它们自带的 `position: fixed` 重置为 `static`，改由 Base UI 定位器接管。
- **受控为主。** 我们的弹窗/菜单都由外部 state 驱动（挂载即开），用 `open` + `onOpenChange`，不用 `*.Trigger`。Esc / 外部点击关闭、焦点陷阱与恢复都由 Base UI 提供。`Modal` 默认把初始焦点投到 body 内首个控件（避免落到 header 的 ✕）。
