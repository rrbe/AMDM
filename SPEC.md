...
- **性能加固** ✅ 已完成
  ...
  - **侧边栏目录树虚拟化 vs sticky 段头(待解张力)**:目录树当前为 `rows.map()` 直渲、未虚拟化,与铁律 #1(所有大列表/树一律虚拟化)有差距。连接行的 sticky-scroll 段头(`position: sticky; top: 0`,见 `styles.css` 的 `.conn-item`)**结构上依赖真实 DOM 流**,虚拟化常用的绝对定位容器会令其失效。将来若要虚拟化目录树,需同步设计 sticky 段头的替代实现(如 `@tanstack/react-virtual` 的 sticky-index 方案),二者不可简单叠加。
